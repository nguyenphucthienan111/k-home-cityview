/**
 * parse-news.mjs — Tự động đọc api/news.ts và trả về array metadata
 * Dùng bởi: generate-static-html.mjs và generate-sitemap.mjs
 *
 * Không cần maintain file riêng — mỗi lần thêm bài vào api/news.ts
...
const NEWS_TS   = path.join(__dirname, "../api/news.ts");

/**
 * Parse api/news.ts và trả về array:
...
export function parseNewsData() {
  ...
  // Tìm tất cả block object trong DATA array.
  ...
  if (entries.length === 0) {
    throw new Error(
      "parse-news.mjs: Không tìm thấy bài nào trong api/news.ts. " +
